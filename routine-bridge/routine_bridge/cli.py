...
OpenClaw 에이전트가 서브프로세스로 호출하거나, 사람이 직접 쓸 수 있다.
    ...
    echo "이 함수 리뷰해줘" | python -m routine_bridge ask --stdin
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="routine_bridge",
        description="Routines API 트리거로 Claude 세션을 띄우고 GitHub 메일박스 댓글로 답을 받는다.",
    )
    sub = parser.add_subparsers(dest="command", required=True)

    ask_p = sub.add_parser("ask", help="Routine 을 발화시키고 답변 댓글을 받는다.")
